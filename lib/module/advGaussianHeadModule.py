...
class advGaussianHeadModule(nn.Module):
    def __init__(self,  cfg, xyz, feature, landmarks_3d_neutral, add_mouth_points=False):
        super(advGaussianHeadModule, self).__init__()
        ...
        # 新增对抗MLP (输入维度需匹配feature_dim + exp/pose_dim)
        self.advexp_color_mlp = MLP(cfg.exp_color_mlp, last_op=None)
        self.advpose_color_mlp = MLP(cfg.pose_color_mlp, last_op=None)

        self._pretrained_modules = ['xyz','exp_color_mlp', 'pose_color_mlp','feature','exp_deform_mlp','pose_deform_mlp','exp_attributes_mlp','pose_attributes_mlp','scales','rotation','opacity']
    ...
    def generate(self, data):
        ...
        exp_weights = torch.clamp(
            (self.dist_threshold_far - dists) / (self.dist_threshold_far - self.dist_threshold_near), 0.0, 1.0)
        ...
        adv_color = torch.zeros_like(color, device=xyz.device)  # 新增对抗颜色

        delta_xyz = torch.zeros_like(xyz, device=xyz.device)
        delta_attributes = torch.zeros(
            [B, xyz.shape[1], self.scales.shape[1] + self.rotation.shape[1] + self.opacity.shape[1]], device=xyz.device)
        for b in range(B):
            feature_exp_controlled = feature[b, exp_controlled[b], :]
            exp_color_input = torch.cat([feature_exp_controlled.t(),
                                         data['exp_coeff'][b].unsqueeze(-1).repeat(1, feature_exp_controlled.shape[0])],
                                        0)[None]
            exp_color = self.exp_color_mlp(exp_color_input)[0].t()
            color[b, exp_controlled[b], :] += exp_color * exp_weights[b, exp_controlled[b], :]

            # 新增对抗颜色计算 - 使用相同的输入但不同的MLP
            adv_exp_color = self.advexp_color_mlp(exp_color_input)[0].t()
            adv_color[b, exp_controlled[b], :] += adv_exp_color * exp_weights[b, exp_controlled[b], :]

            feature_pose_controlled = feature[b, pose_controlled[b], :]
            pose_color_input = torch.cat([feature_pose_controlled.t(),
                                          self.pos_embedding(data['pose'][b]).unsqueeze(-1).repeat(1,
                                                                                                   feature_pose_controlled.shape[
                                                                                                       0])], 0)[None]
            ...
            # # 对抗颜色生成（添加噪声输入）
            # adv_input = torch.cat([
            #     feature_exp_controlled.t(),
            #     torch.randn(1, feature_exp_controlled.shape[0], device=feature.device) * 0.1  # 噪声输入
            # ], 0)[None]
            #
            # # 生成归一化扰动（tanh输出范围[-1,1]）
            # adv_color[b, exp_controlled[b], :] = self.adv_color_mlp(adv_input)[0].t()
            #新增对抗颜色计算 - 姿态部分
            adv_pose_color = self.advpose_color_mlp(pose_color_input)[0].t()
            adv_color[b, pose_controlled[b], :] += adv_pose_color * pose_weights[b, pose_controlled[b], :]
            ...
            exp_deform_input = torch.cat([self.pos_embedding(xyz_exp_controlled).t(),
                                          data['exp_coeff'][b].unsqueeze(-1).repeat(1, xyz_exp_controlled.shape[0])],
                                         0)[None]
        ...
        data['adv_color'] = adv_color
        data['adv_output'] = color + adv_color
        return data

    def load_pretrained(self, state_dict):
        # 步骤1：过滤出预训练模块的参数
        pretrained_dict = {k: v for k, v in state_dict.items()
                           if any(m in k for m in self._pretrained_modules)}
        # 步骤2：获取当前模型的状态字典
        model_dict = self.state_dict()
        # 步骤3：更新仅预训练模块的参数
        model_dict.update(pretrained_dict)
        # 步骤4：严格加载（确保预训练参数完全匹配）
        self.load_state_dict(model_dict, strict=False)
        # 冻结预训练参数（可选）
        for name, param in self.named_parameters():
            if any(m in name for m in self._pretrained_modules):
                param.requires_grad = False
        print(f"Loaded {len(pretrained_dict)}/{len(state_dict)} pretrained params")