...
class MeshHeadModule(nn.Module):
    def __init__(self, cfg,cfg2, init_landmarks_3d_neutral):
        ...
        self.pos_embedding2, _ = get_embedder(cfg2.pos_freq)
    ...
    #myself attack
        self.v_geo_mlp = MLP(cfg2.geo_mlp, last_op=nn.Tanh())
    ...
    def reconstruct(self, data):
        ...
        geo_input2 = self.pos_embedding2(query_pts).permute(0, 2, 1)

        pred2 = self.geometry(geo_input2)

        sdf2, deform2, features2 = pred2[:, :1, :], pred2[:, 1:4, :], pred2[:, 4:, :]
        sdf2 = sdf2.permute(0, 2, 1)
        features2 = features2.permute(0, 2, 1)
        verts_deformed2 = (query_pts + torch.tanh(deform2.permute(0, 2, 1)) / self.grid_res)
        verts_list, features_list, faces_list = marching_tetrahedra(0.5*verts_deformed+0.5*verts_deformed2, 0.5*features+0.5*features2, self.tets, 0.5*sdf+0.5*sdf2)

        #--------------------------------------------------------------------
        # alpha = torch.tensor(0.5)
        # verts_list=alpha*verts_list[0]+(1-alpha)*verts_list2[0]
        # features_list=alpha*features_list[0]+(1-alpha)*features_list2[0]
        # faces_list=alpha*faces_list[0]+(1-alpha)*faces_list2[0]
        # print(verts_list)
