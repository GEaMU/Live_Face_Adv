...
class GaussianHeadModule(nn.Module):
    def __init__(self,indexx, cfg, xyz, feature, landmarks_3d_neutral, add_mouth_points=False):
    ...
    def generate(self, data):
        ...
        # # 获取x坐标并归一化到[-1,1]范围
        # x_coords = xyz[:, :, 0:1]  # 取x坐标并保持维度
        # x_normalized = (x_coords - x_coords.min()) / (x_coords.max() - x_coords.min()) * 2 - 1  # 归一化到[-1,1]
        # # 根据x坐标调整opacity
        # rotation = rotation + x_normalized * 0.5  # 0.5是调整强度系数，可根据需要修改
        ...
        # x_normalized = (x_coords - x_coords.min()) / (x_coords.max() - x_coords.min()) * 2 - 1  # 归一化到[-1,1]
        # # 根据x坐标调整opacity
        # opacity = opacity + x_normalized * 0.5  # 0.5是调整强度系数，可根据需要修改

        # # 获取color矩阵的上下限
        # min_val = opacity.min()  # 0
        # max_val = opacity.max()  # 0
        #
        # # 计算中间值
        # mid_val = min_val + (max_val - min_val) / 2  # 0
        #
        # # 生成与原color大小相同的矩阵，填充mid_val
        # mid_matrix = torch.full_like(opacity, fill_value=mid_val, device=xyz.device)
        # opacity=opacity+0.5*mid_matrix
        ...
        if 'pose' in data:
            ...
            # S = data['scale'][:, :, None]  # [B, 3, 1]
            # x_offset = 1.1  # 例如，X 轴缩放增加 10%
            # S[:, 0, :] *= x_offset  # 修改 X 轴缩放
            ...
            #
            # # 生成与原color大小相同的矩阵，填充mid_val
            # mid_matrix = torch.full_like(S, fill_value=mid_val, device=xyz.device)
            # S=S+0.5*mid_matrix
            ...
            # # 在生成rotation后添加x轴旋转
            # x_rotation_offset = 0.5  # 弧度值
            # # 将四元数转换为旋转矩阵
            # rotation_matrix = quaternion_to_matrix(rotation)
            # # 创建x轴旋转矩阵
            # x_rotate = torch.tensor([
            #     [1, 0, 0],
            #     [0, math.cos(x_rotation_offset), -math.sin(x_rotation_offset)],
            #     [0, math.sin(x_rotation_offset), math.cos(x_rotation_offset)]
            # ], device=rotation_matrix.device)
            # # 应用旋转偏移
            # rotation_matrix = torch.einsum('bnij,jk->bnik', rotation_matrix, x_rotate)
            # # 转换回四元数
            ...
            #
            # # 获取color矩阵的上下限
            # min_val = rotation.min()  # 0
            # max_val = rotation.max()  # 0
            #
            # # 计算中间值
            # mid_val = min_val + (max_val - min_val) / 2  # 0
            #
            # # 生成与原color大小相同的矩阵，填充mid_val
            # mid_matrix = torch.full_like(rotation, fill_value=mid_val, device=xyz.device)
            # rotation=rotation+mid_matrix
        ...
        # 计算中间值
        mid_val = min_val + (max_val - min_val) / 10  # 0


        #生成与原color大小相同的矩阵，填充mid_val
        mid_matrix = torch.full_like(color, fill_value=mid_val, device=xyz.device)
        color=color+1*mid_matrix*self.indexx#
        # scale=0.0001
        # perturbation=torch.randn_like(color)+scale
        data['color'] = color#+perturbation
