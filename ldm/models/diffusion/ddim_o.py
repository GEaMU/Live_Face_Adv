    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None:
                ...
                # ========== 步骤1：将img_orig和mask缩放到64x64（与img尺寸对齐） ==========
                # img_orig（图像类）用bilinear插值，mask（二值类）用nearest插值
                img_orig = torch.nn.functional.interpolate(img_orig, size=(64, 64), mode='bilinear',
                                                           align_corners=False)
                mask = torch.nn.functional.interpolate(mask, size=(64, 64), mode='nearest')  # 避免mask出现非0/1值

                # ========== 步骤2：统一通道数到4（匹配img的4通道） ==========
                # 1. img_orig扩展到4通道（补第4通道为0）
                img_orig = torch.nn.functional.pad(img_orig, (0, 0, 0, 0, 0, 1))  # [1,3,64,64] → [1,4,64,64]
                # 2. mask扩展到4通道（重复1通道到4通道）
                mask = mask.repeat(1, 1, 1, 1)  # [1,1,64,64] → [1,4,64,64]
